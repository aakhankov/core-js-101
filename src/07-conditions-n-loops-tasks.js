...
function getFizzBuzz(num) {
  // let result;
  // return num % 3 === 0 ? 'Fizz' : num || num % 5 === 0 ? 'Buzz' : num;
...
//   // Номер кредитной карты валиден, если сумма делится на 10 нацело
//   return sum % 10 === 0;
// }


function isCreditCardNumber(ccn) {
  const cardNumberArr = ccn.toString().split('').map(Number).reverse();
  let result = 0;
  for (let i = 0; i < cardNumberArr.length; i += 1) {
    let item = cardNumberArr[i];
    if (i % 2 === 1) {
      item *= 2;
      if (item > 9) {
        item -= 9;
...
function getDigitalRoot(num) {
  const numbersString = num.toString();
  let sum = 0;
 ...
 * const part = parts[0][i];
    if (parts.every(path => path[i] === part)) {
        commonPath += (i === 0 ? '' : '/') + part;
    } else {
        break;
    }

    0:(4) ['', 'web', 'images', 'image1.png']
    1:(4) ['', 'web', 'images', 'image2.png']
 */
function getCommonDirectoryPath(pathes) {
  const partsOfPath = pathes.map((path) => path.split('/'));
  let commonPath = '';
  for (let i = 0; i < partsOfPath[0].length; i += 1) {
    const part = partsOfPath[0][i];
    if (partsOfPath.every((path) => path[i] === part)) {
      commonPath += `${part}/`;
    } else {
      break;
    }
